...
def _make_client(tmp_path):
    return NaobClient(cache_path=tmp_path / "naob_cache.sqlite", min_interval=0)
...
def test_search_no_hits_redirect_returns_empty_without_following(tmp_path, monkeypatch):
    client = _make_client(tmp_path)
    try:
        calls = []
...
def test_search_single_hit_redirect_returns_one_result(tmp_path, monkeypatch):
    ...
    try:
        monkeypatch.setattr(
            client, "_get", lambda *a, **k: _fake_response(307, headers={"Location": "/ordbok/jul"})
        )
...
def test_get_entry_404_returns_none(tmp_path, monkeypatch):
    client = _make_client(tmp_path)
    try:
